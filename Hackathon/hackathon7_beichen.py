...
ds = tfds.load('mnist', data_dir = '/work/tadesse/beichen/2020S_hackathons',shuffle_files=True) # this loads a dict with the datasets
...
encoder_network = tf.keras.Sequential([
    ...
    tf.keras.layers.Conv2D(64, 3, padding='same',
                           activation=tf.nn.swish),  # 14,14,64
    tf.keras.layers.Conv2D(16, 3, padding='same',
                           activation=tf.nn.swish),  # 14,14,16
    tf.keras.layers.Conv2D(8, 3, padding='same'),  # 14,14,8
])

decoder_network = tf.keras.Sequential([
    tf.keras.layers.Conv2D(8, 3, padding='same',
                           activation=tf.nn.swish),  # 14,14,8
    tf.keras.layers.Conv2D(16, 3, padding='same',
                           activation=tf.nn.swish),  # 14,14,16
    tf.keras.layers.Conv2D(64, 3, padding='same',
                           activation=tf.nn.swish),  # 14,14,64
