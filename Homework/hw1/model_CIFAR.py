...
class ru_ResNET34(tf.keras.layers.Layer):
    def __init__(self, filters, strides = 1, activation = 'relu', **kwargs):
        ...
        self.main_layers = [
            tf.keras.layers.Conv2D(filters, 3, strides = strides, padding = 'same', kernel_regularizer= tf.keras.regularizers.l2(0.01),use_bias = False),
            tf.keras.layers.BatchNormalization(),
            self.activation,
            tf.keras.layers.Conv2D(filters, 3, strides = 1, padding = 'same', kernel_regularizer= tf.keras.regularizers.l2(0.01), use_bias = False),
            tf.keras.layers.BatchNormalization()
        ]
        self.skip_layers = []
        if strides > 1:
