...
es_callback =EarlyStopping(monitor='loss', min_delta=0.0001, patience = 10)
cp_callback = ModelCheckpoint(filepath=checkpoint_path, save_weights_only=True, verbose=0, save_freq='epoch')
training = ResNet.fit(X_train,y_train_encoded, batch_size = 300, epochs = 100, callbacks = [es_callback,cp_callback],validation_data = (X_val,y_val_encoded), verbose = 1)
